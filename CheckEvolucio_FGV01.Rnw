...
\hline
 &  & \\
1 & 15-May-2022 & Replicant el document ``Evolucio FG.html'' amb les dades que em van enviar i que pentanyen als diabètics\\
 &  & \\
&  & \\
\hline
\end{tabular}
...
<<echo=TRUE, results='markup', warning=FALSE, error=FALSE, message=FALSE, size="footnotesize", out.width="50%">>=
library(sjlabelled)
